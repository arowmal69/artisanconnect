...
import { ChevronRight, LayoutDashboard, ShoppingBag, PlusCircle, Star } from 'lucide-react';
    ...
    <main className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8 py-8 h-full bg-slate-50 min-h-screen">
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage your active commissions, orders, and creative profile</p>
        </div>
        <Link href="/explore">
           <Button className="bg-violet-600 hover:bg-violet-700 text-white shadow-sm font-semibold rounded-xl">
             <PlusCircle className="h-4 w-4 mr-2" /> Find Freelancers
           </Button>
        </Link>
      </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="rounded-2xl bg-white p-6 border border-slate-200 flex flex-col items-center text-center shadow-sm">
            <img
              ...
              className="h-24 w-24 rounded-full object-cover mb-4 ring-4 ring-violet-50"
            />
            <h3 className="text-lg font-bold text-slate-900">{profile.full_name || profile.username}</h3>
            <Badge variant="secondary" className="mt-1 capitalize bg-violet-50 text-violet-700 border border-violet-100">{profile.role || 'User'}</Badge>
          ...
          <div className="space-y-4">
            {activeOrders.length === 0 ? (
              <div className="flex flex-col items-center justify-center bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
                 ...
                 <p className="text-sm text-slate-500 mt-2 max-w-sm mx-auto">You don't have any active commission orders yet. Discover talented freelancers to get started.</p>
                 <Link href="/explore" className="mt-6">
                    <Button variant="outline" className="font-semibold rounded-xl border-slate-200 hover:bg-slate-50">Explore Services</Button>
                 </Link>
              </div>
            ) : (
              activeOrders.map((order) => {
                ...
                return (
                  <div key={order.id} className="rounded-2xl bg-white border border-slate-200 p-6 flex flex-col shadow-sm hover:border-violet-200 transition-colors">
          ...
          <div className="space-y-4">
            {recommendedServices.length > 0 ? recommendedServices.map((service, i) => (
              <Link key={service.id} href={`/services/${service.id}`} className="block">
                <div className="rounded-2xl bg-white border border-slate-200 overflow-hidden shadow-sm hover:border-violet-300 hover:shadow-md transition-all group">
                  <div className="h-32 bg-slate-100 overflow-hidden relative">
                     {service.image_url ? (
                       ...
                       <img src={`https://images.unsplash.com/photo-1579783902614-a3fb3927b6a5?w=400&q=80&random=${i}`} alt={service.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                     )}
                  ...
                  <div className="p-4">
                    ...
                    <h4 className="text-sm font-bold text-slate-900 line-clamp-2 mb-2 group-hover:text-violet-600 transition-colors">{service.title}</h4>
                    <div className="flex items-center gap-1 text-xs font-bold text-amber-500">
                       <Star className="h-3 w-3 fill-current" /> 5.0
                       <span className="text-slate-400 font-medium ml-1">(Top Rated)</span>
                    </div>
                  </div>
                </div>
              </Link>
